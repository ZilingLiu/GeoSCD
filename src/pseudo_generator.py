...
class PseudoGenerator(nn.Module):
    ...
    def forward(self, inputs, save_key = False):
        ...
        if save_key:
            k0, k1 = self._get_key(input_t0_qkv, input_t1_qkv)
            
            k0 = k0.permute(0, 3, 1, 2)  # shape [16, 80, 32, 32]
            k0 = F.interpolate(k0, size=self.img_size, mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
            
            k1 = k1.permute(0, 3, 1, 2)  # shape [16, 80, 32, 32]
            k1 = F.interpolate(k1, size=self.img_size, mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
            
            return embed_t0, embed_t1, k0, k1
        else:
            return embed_t0, embed_t1, key, query, value
        
    def _generate(self, input_t0_qkv, input_t1_qkv):
        ## multi-head feature correlation n multi-head feature dim
    ...
    def _get_key(self, input_t0_qkv, input_t1_qkv):
        input_t0_key = input_t0_qkv[1, :, :, :, :]
        input_t1_key = input_t1_qkv[1, :, :, :, :]
        ...
        input_t0_key = rearrange(input_t0_key, 'b n (h w) c -> b n h w c', h=h, w=w)
        input_t1_key = rearrange(input_t1_key, 'b n (h w) c -> b n h w c', h=h, w=w)
        return input_t0_key[0], input_t1_key[0]