...
def visualize_results(rgb_img_t0, rgb_img_t1, final_change_mask, gt=None, resize_hw = None):
...
def visualize_results_custom(rgb_img_t0, rgb_img_t1, final_change_mask, gt=None, resize_hw=None, save_path="visualization.png"):
    """
    将输入图像、预测mask、可选gt mask resize 并拼接保存，不显示。
    
    Args:
        rgb_img_t0 (np.ndarray): t0图像 (H, W, 3)
        rgb_img_t1 (np.ndarray): t1图像 (H, W, 3)
        final_change_mask (np.ndarray): 预测的变化掩码 (H, W)
        gt (np.ndarray, optional): Ground truth mask
        resize_hw (tuple, optional): (height, width) 目标分辨率
        save_path (str): 保存的图像路径
    """
    # Resize 所有输入
    if resize_hw is not None:
        rgb_img_t0 = cv2.resize(rgb_img_t0, (resize_hw[1], resize_hw[0]))
        rgb_img_t1 = cv2.resize(rgb_img_t1, (resize_hw[1], resize_hw[0]))
        final_change_mask = cv2.resize(final_change_mask, (resize_hw[1], resize_hw[0]), interpolation=cv2.INTER_NEAREST)
        if gt is not None:
            gt = cv2.resize(gt, (resize_hw[1], resize_hw[0]), interpolation=cv2.INTER_NEAREST)
